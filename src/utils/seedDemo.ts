...
async function seedDemoWorkflow() {
  ...
  const nodes = [
    ...
    { id: 'classify', type: WorkflowStepType.AI_CLASSIFICATION, label: 'Priority Routing', position: { x: 850, y: 300 }, configuration: { prompt: 'Classify priority. If text contains PRODUCTION DOWN, priority MUST be "CRITICAL". Otherwise "LOW".' } },
    { id: 'condition', type: WorkflowStepType.DETERMINISTIC_CONDITION, label: 'Escalation Check', position: { x: 1100, y: 300 }, configuration: { field: 'classify.classification.priority', operator: 'equals', value: 'CRITICAL' } },
  ...
  console.log(`Created Workflow: Support Ticket Triage (ID: ${workflow._id})`);

  // 2. Create 2 Sample Runs for easy Rerunning
  const sample1 = {
    source: 'api',
    payload: {
  ...
  console.log('Created Routine Ticket Sample (FALSE Branch)');
  ...
  console.log('Created Critical Outage Sample (TRUE Branch)');

  console.log('Demo seeded successfully!');
  process.exit(0);
}

seedDemoWorkflow().catch(console.error);
